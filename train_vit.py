...
def train_step(model, dataloader, criterion, optimizer, device, name):
    ...
    for i, data in enumerate(dataloader):
        ...
        print(
            f"\rTraining: [{i+1}/{len(dataloader)}] "
            f"Loss: {train_loss / (i + 1):.4f} "
            f"Acc: {train_acc / (i + 1):.4f}",
            end="",
        )
...
def eval_step(model, dataloader, criterion, device, name):
    ...
    for i, data in enumerate(dataloader):
        ...
        # Print dynamic progress on the same line using \r
        print(
            f"\rEvaluation: [{i+1}/{len(dataloader)}] "
            f"Loss: {eval_loss / (i + 1):.4f} "
            f"Acc: {eval_acc / (i + 1):.4f}",
            end="",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="vit_smal1_16",
        help="Either vit_small_16/32 or vit_base_16/32 or vit_large_16/32. Default is vit_small_16.",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        help="Path to the directory where training log and model will be saved. Will be saved in {output_dir}/model.",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        help="Either mnsit, cifar-10 or cifar-100 or PACS. Default is cifar-10.",
        default="cifar-10",
    )
    parser.add_argument(
        "--save_model",
        type=bool,
        help="Specify whether the trained model must be saved or not. Will be saved in {output_dir}/model.",
        default=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ft_type",
        type=str,
        help="Specify which parts of the model to finetune. Choose from full or classifier. Default is classifier.",
        default="classifier",
    )
    ...
    train_ds, test_ds = get_dataset_func(args.dataset)(
        root=f"./{args.out_dir}/{args.dataset}"
    )

    if args.dataset == "PACS":
        train_dl, test_dl = get_PACS_dataloader(
            train_ds, test_ds, batch_size, num_workers
        )
    ...
    model = load_vit(args.model_name, args.ft_type)
    model.to(device)
    ...
    with open(os.path.join(args.out_dir, "log/run.csv"), "w") as f:
        ...
        for epoch in tqdm(
            ...
            train_loss, train_acc = train_step(
                model, train_dl, criterion, optimizer, device, args.dataset
            )
            test_loss, test_acc = eval_step(
                model, test_dl, criterion, device, args.dataset
            )
            ...
            tqdm.write(
                f"============ Epoch {epoch + 1} --> Train Acc: {train_acc:.4f} || Test Acc: {test_acc:.4f} || Time: {epoch_duration:.2f} s ============\n"
            )
